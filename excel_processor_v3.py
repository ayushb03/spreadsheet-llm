...
class SpreadsheetLLM:
    ...
    def structural_anchor_extraction(self, k: int = 4) -> pd.DataFrame:
        ...
        non_empty_cols = [self.spreadsheet.columns.get_loc(col) 
                         for col in self.spreadsheet.columns[self.spreadsheet.notna().any(axis=0)]]
        
        non_empty_rows = self.spreadsheet.index[self.spreadsheet.notna().any(axis=1)].tolist()
